  ...
  switch (expectedType) {
    case 'flashcards':
      ...
      parsed.flashcards.forEach((card, index) => {
        if (!card.id || !card.front || !card.back) {
          throw new Error(`Invalid flashcard at index ${index}: missing required fields (id, front, back)`);
        }
        if (!['easy', 'medium', 'hard'].includes(card.difficulty)) {
          throw new Error(`Invalid flashcard at index ${index}: difficulty must be "easy", "medium", or "hard"`);
        }
    ...
    case 'quiz':
      ...
      parsed.quiz.forEach((question, index) => {
        if (!question.id || !question.question || !question.options || !question.correct || !question.explanation) {
          throw new Error(`Invalid quiz question at index ${index}: missing required fields`);
        }
        if (!Array.isArray(question.options) || question.options.length !== 4) {
          throw new Error(`Invalid quiz question at index ${index}: options must be an array of exactly 4 items`);
        }
        if (!question.options.includes(question.correct)) {
          throw new Error(`Invalid quiz question at index ${index}: correct answer must be one of the options`);
        }
      });
      break;
    ...
    case 'summary':
      ...
      if (!Array.isArray(summary.keyConcepts) || !Array.isArray(summary.mainTopics) ||
          !Array.isArray(summary.importantTerms) || !summary.overview) {
