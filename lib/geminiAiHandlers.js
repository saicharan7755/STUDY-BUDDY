...
export async function dispatchGemini(ai, endpoint, body) {
  ...
  switch (endpoint) {
    ...
    case 'true-false': {
      ...
      const count = typeof body.count === 'number' ? body.count : 5;
      if (!topic) {
        throw new AiRouteError(400, 'Missing true/false topic.');
      }
      const prompt = TRUE_FALSE_PROMPT.replace('{topic}', topic).replace('{count}', count);
      const contents = [{ role: 'user', parts: [{ text: prompt }] }];
      const response = await callGemini(ai, { contents, expectJson: true });
      return parseJSONResponse(response.text, 'true-false');
    }
    ...
    case 'deeper': {
      ...
      const prompt = DEEPER_EXPLANATION_PROMPT.replace('{topic}', topic).replace(
        '{currentSummary}',
        currentSummary
      );
      const contents = [{ role: 'user', parts: [{ text: prompt }] }];
